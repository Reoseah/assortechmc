...
public abstract class CableBlock extends BlockWithEntity implements EU.ElectricBlock {
    ...
    protected boolean connectsTo(BlockView view, BlockPos pos, Direction side) {
        ...
        if (block instanceof NormalCableBlock) {
            return true;
        }
    ...
    public BlockState getStateForNeighborUpdate(BlockState state, Direction direction, BlockState newState, WorldAccess world, BlockPos pos, BlockPos posFrom) {
        if (world instanceof World) {
            CableNetwork.of((World) world).onCableUpdate(pos);
        }
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return world.isClient ? null : checkType(type, SpaceFactory.BlockEntityTypes.CABLE, CableBlockEntity::tick);
    }
    ...
    public void onPlaced(World world, BlockPos pos, BlockState state, @Nullable LivingEntity placer, ItemStack itemStack) {
        ...
        CableNetwork.of(world).onCableUpdate(pos);
    }
