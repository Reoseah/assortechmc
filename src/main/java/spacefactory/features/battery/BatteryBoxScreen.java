...
public class BatteryBoxScreen extends HandledScreen<BatteryBoxScreenHandler> {
    ...
    protected void drawBackground(MatrixStack matrices, float delta, int mouseX, int mouseY) {
        ...
        if (this.isPointWithinBounds(105, 39, 20, 10, mouseX, mouseY)) {
            this.renderTooltip(matrices, new TranslatableText("tooltip.spacefactory.energy_and_capacity", this.handler.getEnergy(), SpaceFactory.config.redstoneBatteryCapacity).formatted(Formatting.GRAY), mouseX, mouseY);
        }
    }
}
