...
public abstract class CraftingMachineScreen<T extends CraftingMachineScreenHandler> extends HandledScreen<T> {
    ...
    protected void drawMouseoverTooltip(MatrixStack matrices, int x, int y) {
        if (this.isPointWithinBounds(54, 36, 14, 14, x, y)) {
            this.renderTooltip(matrices, new TranslatableText("container.spacefactory.energy_storage", this.handler.getEnergy(), CraftingMachineBlockEntity.CAPACITY).formatted(Formatting.GRAY), x, y);
        }
